export default (aclConfig = {}) => {
  function evaluate(rule, subject, ...args) {
    if (typeof rule === 'function') {
      ...
      user.menu = this.$store.getters['auth/userMenuMap'];
      // user.role = user.role || {};
      // user.isAdmin = store.getters['auth/isAdmin'];
      return rule(user, subject, ...args);
    } else if (rule === undefined) {
      throw new Error(
        `[withAcl]: no rule found for your verb, provided config: ${JSON.stringify(aclConfig)}`
      );
    } else {
      return !!rule;
    }
